...
displayhelp() {
	...
	printf "There are many programs which can be provided either by ${blue}toybox${clear_color} or by ${pink}busybox${clear_color}. Choose which to link to with -p ${pink}busybox${clear_color} or -p ${blue}toybox${clear_color}.\n"
	exit 1
}
...
dropbearbins="dropbear dropbearkey dropbearconvert ssh-keygen dbclient ssh scp"
for b in $busyboxbins; do
	if [ ! -f "$installpath/$b" ]; then
		printf "Linking $binpath/${pink}busybox${clear_color} at $installpath/${pink}$b${clear_color}\n"
		ln -s $binpath/busybox $installpath/$b
	else
		printf "$binpath/${red}$b${clear_color} already exists, skipping\n"
	fi
done
for d in $dropbearbins; do
	if [ ! -f "$installpath/$d" ]; then
		printf "Linking $binpath/${green}dropbearmulti${clear_color} at $installpath/${green}$d${clear_color}\n"
		ln -s $binpath/dropbearmulti $installpath/$d
	else
		printf "$binpath/${red}$d${clear_color} already exists, skipping\n"
	fi
done
for t in $toyboxbins; do
	if [ ! -f "$installpath/$t" ]; then
		printf "Linking $binpath/${blue}toybox${clear_color} at $installpath/${blue}$t${clear_color}\n"
		ln -s $binpath/toybox $installpath/$t
	else
		printf "$binpath/${red}$t${clear_color} already exists, skipping\n"
	fi
done
if [[ "$preference" == "toybox" ]]; then
	for c in $commonbins; do
		if [ ! -f "$installpath/$c" ]; then
			printf "Linking $binpath/${blue}toybox${clear_color} at $installpath/${blue}$c${clear_color}\n"
			ln -s $binpath/toybox $installpath/$c
		else
			printf "$binpath/${red}$c${clear_color} already exists, skipping\n"
		fi
	done
elif [[ "$preference" == "busybox" ]]; then
	for c in $commonbins; do
		if [ ! -f "$installpath/$c" ]; then
			printf "Linking $binpath/${pink}busybox${clear_color} at $installpath/${pink}$c${clear_color}\n"
